...
import {
  requestRegisteredUserEvents,
  retrieveRegisteredUserEvents,
  retrieveRegisteredUserEventsError,
...
const Chat = () => {
  ...
  const userRegisteredEvents = useSelector(
    (state) => state.userRegisteredReducer
  );
  const actualChatParticipants = useSelector(
    (state) => state.chatReducer.actualParticipants
  );
  ...
  const checkOnlineParticipant = (id) => {
    ...
    if (chatInfo.status === "retrieved" && chatInfo.selectedRoom) {
      ...
      if (currentRoom) {
        let activeParticipant = currentRoom.chatParticipants.find(
          (participant) => {
  ...
  return (
    <BigWrapper>
      <Wrapper>
        ...
        {userRegisteredEvents.status !== "retrieved..." ? (
          <div>Seems like you havn't registered for any events!</div>
        ) : (
          ...
          <RoomWrapper>
            {userRegisteredEvents.registeredEvents.map((event, index) => {
              return (
                <Link
                  to={`/chatJoin/${event._id}/${event.groupName}`}
                  key={event._id}
                >
      ...
      <Wrapper>
        ...
        {actualChatParticipants && chatInfo.status === "retrieved" && (
          <div>
            {Object.values(actualChatParticipants).map((participant) => {
              return (
                <div
                  key={`${participant.parkId}${participant.profileImage}`}

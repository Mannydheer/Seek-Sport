...
const validateDateBooking = (startDate) => {
  ...
  if (bookingMonth >= currentMonth && bookingDay > currentDay) {
    if (selectedTime <= nightLimit && selectedTime >= morningLimit) {
      return true;
    } else {
      return false;
    }
...
const Host = () => {
  ...
  const handleHostInformation = async (event) => {
    ...
    if (
      ...
      if (validDate) {
        ...
        try {
          ...
          console.log(
            err,
            "catch error inside handleHosting in Host component."
          );
        }
      } else {
    ...
    //if any of the cases fail.
      ...
      setError(
        "Make sure all fields have been selected and that a valid time was selected."
      );
    }
  };
  ...
  return (
    <PageContainer>
      ...
      >
        <SelectDiv>
          <InputLabel shrink id="Select a sport">
            {" "}
            Sport
          </InputLabel>
          <SelectTag
          ...
          >
            {sports.map((sport) => {
              return (
                <EachMenuItem value={sport} key={sport}>
                  {sport}
                </EachMenuItem>
              );
            })}
        ...
        <SelectDiv>
          <InputLabel shrink id="Select duration">
            Duration
          </InputLabel>
          <SelectTag
        ...
        <Button type="submit" variant="outlined" className={classes.root}>
          Submit
        </Button>
      </FormWrapper>
      <StyledMessage>
        ...
        {conflictEvent !== null && (
          <EventDetailsWrapper>
            <ChangeBooking>
              Would you like to cancel your booking? If not, pick a different
              time!
            </ChangeBooking>
            {!canceled && (
              <EventDetails

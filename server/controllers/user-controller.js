...
const handleSignUp = async (req, res) => {
  ...
  if (!signUpInfo.user || !signUpInfo.pass || !req.file) {
    ...
    try {
      ...
      let checkForUser = await db
        .collection(collectionUsers)
        .findOne({ username: signUpInfo.user });
      //if you do, success.
      if (!checkForUser) {
        ...
        let createUserEvent = await db
          .collection(collectionUserEvents)
          .insertOne({ _id: ObjectId(userId) });

        Promise.all([r, getUser, createUserEvent]).then(() => {
...
const handleLogin = async (req, res) => {
  console.log(getConnection(), "yooo");
  ...
  if (!loginInfo.user || !loginInfo.pass) {
    ...
    try {
      ...
      let checkForUser = await db
        .collection(collectionUsers)
        .findOne({ username: loginInfo.user });
      //if you do, success.
      if (checkForUser) {
